    ...
    public class PagedList<T> : List<T>
    {
        ...
        public PagedList(List<T> item, int count, int pageNumber, int pageSize)
        {
            MetaData = new MetaData
            {
                TotalCount = count,
                PageSize= pageSize,
                TotalPages= (int)Math.Ceiling(count/ (double)pageSize) // por ejemplo 1.5 lo transorma a 2
            };
            AddRange(item); // agregar los elementos de la coleccion al final de la lista
        }
